...
export function SadhnaChat({ adapter }) {
  ...
  // When true, the current "today" queue flow (askNextInQueue) walks through
  // EVERY active activity instead of skipping ones that already have a
  // value — used by the "✏️ Edit / Review Today's Entries" option so a user
  // revisiting an already-filled day can change any answer.
  ...
  const pushFillMenu = (hasExisting = false) => {
    const options = [
    ...
    if (hasExisting) {
      options.unshift({ label: "✏️ Edit / Review Today's Entries", value: "editToday" });
    }
    pushBlock({ kind: "menu", options });
  };
  ...
  const submitActivity = async (activity, value, { silent = false } = {}) => {
    ...
    // While editing an already-complete day, every activity is already
    // "filled" from the very first re-answer, so the normal all-complete
    // short-circuit would end the edit flow after just one question. Skip
    // it here and let askNextInQueue walk through the rest; the final
    // completion screen still shows once editModeRef clears at the end.
    if (status.allComplete && !editModeRef.current) {
  ...
  const askNextInQueue = async () => {
    while (queueRef.current.length > 0) {
      ...
      if (!editModeRef.current && isFilled(todayMapRef.current.get(id))) continue;
      if (!isEligible(activity, todayMapRef.current, activitiesByIdRef.current)) continue;

      const suggestion = editModeRef.current
        ? todayMapRef.current.get(id)
        : flowRef.current === "yesterday"
        ? yesterdayMapRef.current.get(id)
        : undefined;
      const suggestionLabel = editModeRef.current ? "Currently recorded" : undefined;

      pushBlock({ kind: "activity", activity, suggestion, suggestionLabel });
      return;
    }
  ...
  // ---------------------------------------------------------------------
  const startQueueFlow = (kind, ids, { editMode = false } = {}) => {
  ...
  const handleMainMenuSelect = async (blockId, value) => {
    ...
    if (value === "fillToday") {
      // Explicitly choosing "today" is an explicit day-naming, same as
      // typing "aaj" — free-text from here on should target today again,
      // even if a previous chat turn was filling a different date.
      setNlActiveDate(todayISO());
      await showEncouragingMarksIfAny();
      const hasExisting = getCompletionStatus(activeActivities(), todayMapRef.current).completed > 0;
      pushBot(
        hasExisting
          ? "Would you like to edit what's already recorded, or continue filling?"
          : "How would you like to fill today?"
      );
      pushFillMenu(hasExisting);
      return;
    }
  ...
  const handleFillMenuSelect = async (blockId, value) => {
    updateBlock(blockId, { resolved: true });
    const labelMap = {
      editToday: "✏️ Edit / Review Today's Entries",
    ...
    if (value === "editToday") {
      pushBot("✏️ Let's go through today's Sadhna — change anything you'd like.");
      startQueueFlow("guide", allOrdered.map((a) => a.activity_id), { editMode: true });
      return;
    }
  ...
  const beginParticularDateFlow = async (dateISO) => {
    ...
    // From here on, any free-text message ("kal ki chanting 26 mala" or
    // just plain "26 mala" while this flow is active) should update THIS
    // date, not today — this is what keeps the chat input in sync with
    // whichever date the user picked from the menu.
    setNlActiveDate(dateISO);

    if (seedMap.size > 0) {
      pushBot(`🪷 ${formatDateLabel(dateISO)} already has some Sadhna recorded.`);
      pushActionButtons([
        { label: "✏️ Edit / Review Entries", value: "editDateEntries" },
        { label: "➡️ Continue Filling Remaining", value: "continueDateEntries" },
      ]);
      return;
    }
  ...
  const askNextInDateQueue = () => {
    ...
    while (flow.queue.length > 0) {
      ...
      const activity = activitiesByIdRef.current[id];
      if (!activity) continue;
      if (!flow.editMode && isFilled(flow.map.get(id))) continue;
      ...
      pushBlock({
        ...
        suggestionLabel: flow.editMode ? "Currently recorded" : "Already recorded",
  ...
  const handleActionSelect = async (blockId, value, label) => {
    ...
    if (value === "editDateEntries") {
      const flow = dateFlowRef.current;
      if (flow) flow.editMode = true;
      askNextInDateQueue();
      return;
    }

    if (value === "continueDateEntries") {
      askNextInDateQueue();
      return;
    }
  ...
  const applyNlUpdatesForDate = async (updates, dateISO) => {
    ...
    // it would re-ask (via the queue) for something already answered here
    // through free text.
    const flow = dateFlowRef.current;
