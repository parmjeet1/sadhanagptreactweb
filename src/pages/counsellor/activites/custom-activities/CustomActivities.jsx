...
const CustomActivities = () => {
    ...
    // Read initial navigation state if passed
    const initialCenterId = location.state?.center_id || location.state?.centerId || '';
    const initialLabelId = location.state?.label_id || location.state?.labelId || '';
    ...
    const [selectedGroup, setSelectedGroup] = useState(initialCenterId);
    const [selectedLabel, setSelectedLabel] = useState(initialLabelId);
    ...
    const fetchCenters = useCallback(() => {
        getRequest('/group-subgroup-list', { user_id: userDetails.user_id }, (response) => {
            ...
            if (res && res.code === 200 && Array.isArray(res.data)) {
                setCenters(res.data);
                if (initialCenterId) {
                    const center = res.data.find(c => String(c.center_id) === String(initialCenterId));
                    if (center) setLabels(center.labels || []);
                }
            }
        });
    }, [userDetails.user_id, initialCenterId]);
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0b1628] font-sans pb-32 transition-colors duration-300 flex flex-col relative overflow-x-hidden">
            <AnimatePresence>
                {errorMessage && (
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="fixed top-24 left-0 right-0 z-[100] flex justify-center px-4">
                        <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-6 py-3 rounded-2xl shadow-lg font-bold text-sm border border-red-100 dark:border-red-900/30 text-center max-w-sm">{errorMessage}</div>
                    </motion.div>
                )}
                {successMessage && (
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="fixed top-24 left-0 right-0 z-[100] flex justify-center px-4">
                        <div className="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-6 py-3 rounded-2xl shadow-lg font-bold text-sm border border-green-100 dark:border-green-900/30 text-center max-w-sm">{successMessage}</div>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <header className="sticky top-0 z-40 bg-white/80 dark:bg-[#0b1628]/80 backdrop-blur-md border-b border-gray-200 dark:border-[#112240] flex items-center justify-between px-4 sm:px-6 py-4 transition-all duration-300">
                <div className="flex items-center gap-3 sm:gap-4">
                    <button
                        onClick={() => navigate(-1)}
                        className="w-10 h-10 rounded-full flex items-center justify-center text-gray-500 dark:text-[#6b7a99] hover:bg-gray-100 dark:hover:bg-[#112240] active:scale-90 transition-all shrink-0"
                    >
            ...
            <div className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 py-6 flex flex-col">
                ...
                {/* Selectors Card */}
                <div className="bg-white dark:bg-[#112240] rounded-[20px] p-4 sm:p-5 shadow-sm border border-gray-200 dark:border-[rgba(255,255,255,0.05)] mb-6 flex flex-col sm:flex-row gap-4 items-center transition-all duration-300">
                    <div className="flex-1 w-full relative">
                        ...
                        <select
                            ...
                            className="appearance-none w-full bg-slate-50 dark:bg-[#0b1628] border border-gray-200 dark:border-slate-800 focus:border-teal-500 dark:focus:border-[#1de9b6] rounded-xl px-4 py-2.5 font-bold text-[13px] text-gray-800 dark:text-[#F8FAFC] outline-none transition-colors duration-300 cursor-pointer"
                        >
                ...
                {!selectedGroup ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                        ...
                        <div className="bg-white dark:bg-[#112240] rounded-[20px] p-4 sm:p-6 shadow-sm border border-gray-200 dark:border-[rgba(255,255,255,0.05)] transition-all duration-300">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {assignedActivities.map(student => {
                                        return (
                                            <div
                                                key={student.id}
                                                className="flex items-center gap-2 px-3 py-2 rounded-xl border border-[#22c55e]/30 bg-[#22c55e]/5 text-[#22c55e] hover:bg-[#22c55e]/15 transition-all max-w-full"
                                            >
                        ...
                        <div className="bg-white dark:bg-[#112240] rounded-[20px] p-4 sm:p-6 shadow-sm border border-gray-200 dark:border-[rgba(255,255,255,0.05)] transition-all duration-300">
                            <div className="flex justify-between items-center mb-4 pb-2">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-teal-50 dark:bg-[rgba(29,233,182,0.1)] text-teal-600 dark:text-[#1de9b6] rounded-lg font-bold text-[11px] hover:opacity-85 transition-all shrink-0"
                                >
                            ...
                            <div className="space-y-2 max-h-[450px] overflow-y-auto pr-1 custom-scrollbar">
                                {availableActivities.length === 0 ? (
                                    ...
                                    availableActivities.map(student => {
                                        ...
                                        const isCustom = student.original_status === 3 || student.own_by !== 0;

                                        return (
                                            <div
                                                key={student.id}
                                                onClick={() => toggleStudent(student.id)}
                                                className={`flex items-center justify-between p-3 bg-slate-50 dark:bg-[#0b1628]/40 border cursor-pointer rounded-xl transition-all gap-2 ${isSelected
                                            ...
                                            >
                                                <div className="flex items-center gap-3 min-w-0 flex-1">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h3 className="font-bold text-[14px] text-gray-800 dark:text-white leading-tight truncate">{student.name}</h3>
                                                        <span className="text-[10px] text-gray-400 dark:text-[#6b7a99] capitalize mt-0.5 inline-block truncate">{student.activity_type}</span>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                                                    {isCustom && (
                                                        <button
                                                            ...
                                                            className="w-8 h-8 rounded-lg flex items-center justify-center bg-red-500/10 hover:bg-red-500/20 text-red-500 transition-colors"
                    ...
                    >
                        <div className="flex items-center gap-2">
                            <div>
                                ...
                                <button onClick={() => setSelectedStudents([])} className="text-[11px] text-red-500 dark:text-teal-400 font-bold hover:underline">Clear selection</button>
                            </div>
                        </div>
                        <button
            ...
            {/* Bottom Navigation for Counsellors */}
