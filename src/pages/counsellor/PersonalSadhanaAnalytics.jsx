...
const DailyTooltip = ({ dateStr, value, label, position }) => {
  ...
  return (
    ...
    >
      <div className="bg-[#0f172a]/95 dark:bg-[#1E293B]/95 backdrop-blur-md text-white px-4 py-2 rounded-2xl shadow-2xl relative border border-white/10 dark:border-gray-700 flex flex-col items-center">
        ...
        <div className="absolute -bottom-1 w-3 h-3 bg-[#0f172a] dark:bg-[#1E293B] rotate-45 border-r border-b border-white/5 dark:border-gray-700"></div>
      </div>
    </motion.div>
  );
};
      ...
      <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} className="w-full h-full overflow-visible pointer-events-none">
        ...
        <g className="opacity-60">
          {yTicks.map((yVal, idx) => {
            ...
            return (
              <g key={`y-${idx}`}>
                <line x1={margin.left} y1={yPos} x2={margin.left + width} y2={yPos} stroke="#cbd5e1" className="dark:stroke-gray-600" strokeDasharray="3 3" strokeWidth="1" />
                <text x={margin.left - 8} y={yPos} fontSize="10" fill="#64748b" className="dark:fill-gray-400" textAnchor="end" fontWeight="bold">
                  {splitVal.length > 1 ? (
                    <>
          ...
          {points.map((p, idx) => {
            ...
            return (
              <g key={`x-${idx}`}>
                <line x1={p.x} y1={margin.top} x2={p.x} y2={margin.top + height} stroke="#cbd5e1" className="dark:stroke-gray-600" strokeDasharray="3 3" strokeWidth="1" />
                {d && (
                  <text x={p.x} y={margin.top + height + 14} fontSize="9" fill="#94a3b8" className="dark:fill-gray-400" textAnchor="middle" fontWeight="bold">
...
const PersonalSadhanaAnalytics = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0F172A] font-sans pb-28 relative overflow-x-hidden text-[#0f172a] dark:text-[#F8FAFC] transition-colors duration-300">
      <div className="w-full max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between px-6 pt-10 pb-6">
          <button
            ...
            className="w-12 h-12 rounded-full bg-white dark:bg-[#1E293B] shadow-sm flex items-center justify-center text-[#0f172a] dark:text-[#F8FAFC] border border-gray-50 dark:border-[#334155] active:scale-95 transition-all"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
          </button>

          <div className="flex flex-col items-center">
            <h1 className="text-[20px] font-extrabold text-[#0f172a] dark:text-[#F8FAFC] tracking-tight mt-1">My Sadhana</h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="w-12 h-12 rounded-full bg-white dark:bg-[#1E293B] shadow-sm flex items-center justify-center text-[#0f172a] dark:text-[#F8FAFC] border border-gray-50 dark:border-[#334155] active:scale-95 transition-all relative"
            >
        ...
        <div className="px-6 mb-6">
          <div className="bg-white dark:bg-[#1E293B] p-1.5 rounded-[24px] shadow-sm border border-gray-100 dark:border-[#334155] flex items-center justify-between">
            {tabs.map((tab) => (
              <button
                key={tab}
        ...
        <AnimatePresence>
          {activeTab === 'Custom' && (
            ...
            >
              <div className="bg-white dark:bg-[#1E293B] p-6 rounded-[32px] shadow-sm border border-gray-50 dark:border-[#334155] flex items-center gap-4">
          ...
          {isLoading ? (
            ...
            activitiesData.map((activity, idx) => (
              <motion.div
                ...
                className="bg-white dark:bg-[#1E293B] rounded-[40px] p-8 shadow-[0_15px_40px_rgba(0,0,0,0.03)] border border-gray-50 dark:border-[#334155] flex flex-col relative overflow-visible group"
              >
                {/* Badge */}
                <div className="absolute top-8 right-8">
                  <span className={`px-4 py-1.5 rounded-full text-[12px] font-extrabold flex items-center gap-1.5 ${activity.trend === 'Stable'
                    ? 'bg-[#f0f7ff] dark:bg-blue-950/60 text-[#1a73e8] dark:text-blue-300'
                    : 'bg-[#f0fdf4] dark:bg-emerald-950/60 text-[#16a34a] dark:text-emerald-300'
                    }`}>
                ...
                <div className="mb-2">
                  <span className="text-[12px] font-black text-gray-400 uppercase tracking-[0.14em]">{activity.name}</span>
