...
const CounsellorAddContent = () => {
  ...
  useEffect(() => {
    ...
    getRequest('/center-list-sub-counslor', { user_id: userDetails?.user_id }, (response) => {
  ...
  const fetchPublishedContent = (pageNum = 1, append = false) => {
    ...
    getRequest('/center-list-sub-counslor', payload, (response) => {
      const res = response.data;
      if (res && res.code === 200) {
  ...
  }, [activeTab, viewFilterGroup, viewFilterLabel]);
            ...
            <div className="flex bg-gray-100/80 p-1 rounded-2xl">
              <button 
                ...
                className={`px-5 py-2 rounded-xl text-[13px] font-black transition-all ${
                  activeTab === 'Add' ? 'bg-white text-[#1a73e8] shadow-sm' : 'text-gray-500 hover:text-[#0f172a]'
                }`}
              >
                Create
              </button>
              <button 
                ...
                className={`px-5 py-2 rounded-xl text-[13px] font-black transition-all ${
                  activeTab === 'View' ? 'bg-white text-[#1a73e8] shadow-sm' : 'text-gray-500 hover:text-[#0f172a]'
                }`}
              >
                Library
              </button>
                  ...
                  {centers.map(center => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-[13px] font-bold transition-all border-2 ${
                          isSelected 
                  ...
                  return (
                    <div key={audienceItem.center_id} className="bg-[#f8fafc] rounded-[24px] p-5 border border-gray-100">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[14px] font-extrabold text-[#0f172a]">{center?.name} Labels</span>
                      </div>
                          ...
                          {labels.map(lbl => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-3 py-1.5 rounded-xl text-[12px] font-bold transition-all border-2 ${
                                  isLabelSelected
            ...
            <button 
              onClick={async () => {
                ...
                try {
                  ...
                  // Map 'quote' to 'text' for backend expectation
                  const backendType = contentType === 'quote' ? 'text' : contentType.toLowerCase();
                  ...
                  postRequestWithFile('/add-new-content', formData, (response) => {
                    const { message, type } = processResponse(response.data);
                    if (type === 'success') {
                      setSuccessMessage(message);
                      setTimeout(() => navigate(-1), 2000);
                    } else {
                      setErrorMessage(message || "Failed to publish");
                ...
                {isFetchingContent ? (
                  ...
                  publishedContent.map((item, idx) => (
                    <motion.div 
                      ...
                      className="bg-white rounded-[40px] p-6 shadow-[0_15px_40px_rgba(0,0,0,0.02)] border border-gray-50"
                    >
                      <div className="flex items-center justify-between mb-4">
                        <span className="px-3 py-1 bg-gray-50 rounded-lg text-[10px] font-black text-gray-400 uppercase tracking-widest">
                          {item.type || 'Content'}
                        </span>
                      ...
                      {item.content_type === 'text' || item.type === 'quote' ? (
                        <p className="text-[18px] font-bold text-[#0f172a] leading-relaxed drop-shadow-sm">
                          “{item.content || item.text}”
                        </p>
                      ) : (
                        <div className="relative rounded-3xl overflow-hidden aspect-video bg-gray-100 group">
                           {/* Simplified placeholder for Video/Image in Library */}
                           ...
                           {(item.type === 'video' || item.content_type === 'youtube') && (
                      ...
                      <div className="mt-6 pt-5 border-t border-gray-50 flex items-center justify-between">
                         <div className="flex -space-x-2">
                            {/* Audience Summary Icons */}
                            <div className="w-8 h-8 rounded-full bg-blue-50 border-2 border-white flex items-center justify-center text-blue-500">
                         ...
                         <p className="text-[11px] font-extrabold text-[#1a73e8]">
                            View Details
                         </p>
                      </div>
                    </motion.div>
          ...
          <AnimatePresence>
            {(successMessage || errorMessage) && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 20 }}
                className={`fixed bottom-24 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-2xl shadow-xl flex items-center gap-3 border ${
                  successMessage ? 'bg-green-50 border-green-100 text-green-700' : 'bg-red-50 border-red-100 text-red-700'
                }`}
              >
