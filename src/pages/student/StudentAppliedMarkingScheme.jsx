  ...
  rules.forEach(rule => {
    ...
    if (!scheme[key]) {
      ...
      scheme[key] = {
        ...
        maxMarks: rule.is_max_marks ? rule.marks : 0,
        rows: []
      };
    }
...
const StudentAppliedMarkingScheme = () => {
  ...
  const fetchAppliedRules = () => {
    ...
    getRequest('/applied-marking-scheme', { user_id: userDetails.user_id }, (response) => {
      if (response?.data?.code === 200 && response.data.data) {
        const { scheme_name, applied_level, group_name, subgroup_name, rules } = response.data.data;

        setSchemeMetaData({
          schemeName: scheme_name || 'Applied Marking Scheme',
          appliedLevel: applied_level || 'System Default',
          groupName: group_name,
          subgroupName: subgroup_name
        });

        if (Array.isArray(rules) && rules.length > 0) {
          const mapped = mapDBRulesToDraftScheme(rules);
  ...
  if (isLoading && totalActivities === 0) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0A192F] flex items-center justify-center text-slate-500 dark:text-[#6b7a99]">
        <div className="flex items-center gap-3">
          <div className="w-6 h-6 border-2 border-teal-500 border-t-transparent rounded-full animate-spin"></div>
          <span className="font-semibold text-sm">Loading your applied marking scheme...</span>
        </div>
      </div>
    );
  }
              ...
              <div className="relative mt-2 sm:mt-0 sm:ml-auto">
                ...
                >
                  {availableActivities.length === 0 ? (
                    <option value="">No Activities Assigned</option>
                  ) : (
        ...
        <div className="flex justify-center">
          {selectedActivity ? (
            ...
            <div className="w-full text-center py-12 text-slate-500 dark:text-[#6b7a99] bg-white dark:bg-[#132B5A] border border-dashed border-gray-300 dark:border-[rgba(255,255,255,0.1)] rounded-[16px]">
              ...
              <h3 className="text-[15px] font-bold text-slate-700 dark:text-white mb-1">No activity selected</h3>
              <p className="text-[13px]">Select an activity from the dropdown to inspect its evaluation rules.</p>
            </div>
          )}
        </div>
...
export default StudentAppliedMarkingScheme;
