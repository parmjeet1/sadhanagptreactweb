...
const StudentRanksList = () => {
  ...
  const fetchStudents = useCallback((pageNum = 1, shouldAppend = false) => {
    ...
    getRequest(`/student-rank?${queryString}`, {}, (response) => {
      const res = response.data;
      if (res && res.code === 200) {
        if (res.data?.period) setWeeklyPeriod(res.data.period);
    ...
    <div className="min-h-screen bg-white font-sans pb-[84px]">
      <div className="max-w-md mx-auto">
        ...
        <div className="px-6 pb-2 pt-4 flex flex-col gap-2">
          {weeklyPeriod && (
            <div className="bg-slate-100 dark:bg-slate-800 px-3.5 py-1.5 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-300 flex items-center justify-between">
              <span>🗓️ Last Week Performance</span>
              <span className="text-[#1a73e8] font-black">{weeklyPeriod}</span>
            </div>
          )}
        ...
        <div className="px-2">
          ...
          {isLoading && <div className="text-center py-4 text-gray-500 font-bold">Loading students...</div>}
